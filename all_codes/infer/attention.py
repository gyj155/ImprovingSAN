...
class Attention(nn.Module):
    ...
    def __init__(self, params):
        ...
        self.params = params
        self.channel = params['encoder']['out_channels'] #684
    ...
    def forward(self, cnn_features, hidden, alpha_sum, image_mask=None):
        ...
        query = self.hidden_weight(hidden)#torch.Size([1, 512]
        
        #alpha_sum torch.Size([1, 1, 12, 40])
        alpha_sum_trans = self.attention_conv(alpha_sum) #torch.Size([1, 512, 12, 40])
        
        coverage_alpha = self.attention_weight(alpha_sum_trans.permute(0,2,3,1)) #torch.Size([1, 12, 40, 512])
        
        cnn_features_trans = self.encoder_feature_conv(cnn_features)# [1, 512, 12, 40]
        ...
        alpha = energy_exp / (energy_exp.sum(-1).sum(-1)[:,None,None] + 1e-10) #归一化使得总和为一
