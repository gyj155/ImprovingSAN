...
class SAN_decoder(nn.Module):
    ...
    def forward(self, cnn_features, labels, images_mask, labels_mask, is_train=True):
        ...
        batch_size, num_steps, _ = labels.shape #num_steps = 一个batch中最长的标签长度
        ...
        if is_train:
            ...
            for i in range(num_steps):
                ...
                ############################################################################################################
                hidden_state = self.hidden_state(word_hidden_first)
                c2p_hidden_state = self.c2p_hidden_state(c2p_hidden_first)
                word_out_state = self.final_fusion(torch.cat((current_state, word_weighted_embedding, word_context_weighted, hidden_state), dim=1))
                c2p_out_state = self.c2pfinal_fusion(torch.cat((c2p_state, c2p_weighted_word, c2p_weighted_relation, c2p_context_weighted, c2p_hidden_state), dim=1))
                ############################################################################################################

                if self.params['dropout']:
                    word_out_state = self.dropout(word_out_state)
                    c2p_out_state = self.dropout(c2p_out_state)
            ...
            for i in range(num_steps):
                ...
                if word.item() == 2:
                    ...
                    for num in range(structs.shape[1]-1, -1, -1): #生成的数字 对应符号
        ...
        #print('word_probs:', word_probs, 'struct_probs:', struct_probs, 'word_alphas:', word_alphas, 'c2p_probs:', c2p_probs, 'c2p_alphas:', c2p_alphas)
