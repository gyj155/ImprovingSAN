...
def my_load_checkpoints(new_model, original_weights, init = True):#对于修改过的模型，加载原结构的权重
    ...
    def init_weights(m):
        nonlocal initialized_layers
        if isinstance(m, nn.Linear) or isinstance(m, nn.GRUCell):
            nn.init.xavier_uniform_(m.weight)
            nn.init.zeros_(m.bias)
            initialized_layers += 1
    if init:
        new_model.decoder.final_fusion.apply(init_weights)
        new_model.decoder.c2pfinal_fusion.apply(init_weights)
        new_model.decoder.hidden_state.apply(init_weights)
        new_model.decoder.c2p_hidden_state.apply(init_weights)
        
    print(f'loading succeeded\nmatched_layers: {matched_layers}, new_initialized_layers: {initialized_layers}')